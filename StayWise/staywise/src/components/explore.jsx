...
const Explore = () => {
  ...
  const logInteraction = async (action, motelIds) => {
    try {
      ...
      const response = await axios.post("http://98.81.233.189:5000/log-interaction", payload);
...
const fetchRecommendations = async (userId, city, state) => {
  try {
      ...
      const response = await axios.get(`http://98.81.233.189:5000/recommend-motels/${userId}`);
...
useEffect(() => {
 ...
 // Mapping of state names to abbreviations
  ...
  useEffect(() => {
    ...
    const fetchMotels = async (state) => {
      try {
        ...
        const response = await axios.get(`http://98.81.233.189:5000/motels?state=${state}`);
        // Fetch average ratings for each motel
        const ratings = {};
        for (const motel of response.data) {
          const ratingResponse = await axios.get(`http://98.81.233.189:5000/reviews/averages/${motel.id}`);
  ...
  const handleLocationSearch = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://98.81.233.189:5000/locationSearch?state=${stateAbbreviation}&city=${city}`
      );
